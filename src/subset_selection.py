...
from src.data_utils import load_covtype_data, get_X_y, RANDOM_STATE
...
def _log_likelihood(model, X, y):
    """Sum of log‑probabilities of the true classes."""
...
def _null_log_likelihood(y):
    """Log‑likelihood of an intercept‑only model."""
...
def compute_criteria(model, X, y, p):
    """Return dict with AIC, BIC, Cp, Adj_R2 for a fitted model."""
    n = len(y)
...
def forward_stepwise(X, y, feature_names, max_features=None):
    """
    Greedy forward selection. At each step pick the feature that yields the
    lowest 5‑fold CV error when added.

    Returns list of dicts with per‑step statistics.
    """
    ...
    for step in range(1, max_features + 1):
        ...
        for f in remaining:
            ...
            cv_acc = cross_val_score(model, X_sub, y, cv=5, scoring="accuracy", n_jobs=-1).mean()
        ...
        # Fit final model on full train with selected features
        ...
        history.append(crit)
        print(f"  Step {step:2d} | added '{feature_names[best_feat]}' | "
              f"CV_Err={crit['CV_Error']:.4f}  AIC={crit['AIC']:.1f}  "
              f"BIC={crit['BIC']:.1f}  Adj_R2={crit['Adj_R2']:.4f}")
...
def plot_subset_selection(history, save_path="task2a_subset_selection.png"):
    ...
    for idx, (name, vals) in enumerate(metrics.items()):
        ...
        if name == "Adjusted R²":
            ...
            opt = np.argmin(vals)
        ax.axvline(n_feat[opt], color="red", linestyle="--", alpha=0.6)
    ...
    plt.suptitle("Task 2a — Subset Selection: Criteria vs Number of Predictors",
                 fontsize=14, y=1.01)
    plt.tight_layout()
...
def main():
    print("=" * 60)
    print("Task 2a: Subset Selection Methods")
    print("=" * 60)

    df = load_covtype_data()
    X, y, feature_names = get_X_y(df, continuous_only=True)
    print(f"Using {X.shape[1]} continuous features, {X.shape[0]} samples\n")

    print("Forward Stepwise Selection (5‑fold CV at each step):")
    history = forward_stepwise(X, y, feature_names, max_features=X.shape[1])

    # Report optimal subsets per criterion
    print("\n" + "─" * 55)
    for metric in ["CV_Error", "AIC", "BIC", "Cp"]:
        opt = min(history, key=lambda h: h[metric])
        print(f"  Best by {metric:<10}: p={opt['n_features']}  {metric}={opt[metric]:.4f}")
    opt_r2 = max(history, key=lambda h: h["Adj_R2"])
    print(f"  Best by Adj_R2    : p={opt_r2['n_features']}  Adj_R2={opt_r2['Adj_R2']:.4f}")
    print(f"  Features (Adj_R2) : {opt_r2['features']}")
