...
def pca_component_analysis(X_train, y_train, X_test, y_test, max_comp=None):
    ...
    for nc in range(1, max_comp + 1):
        ...
        print(f"  PCA nc={nc:2d}  CV_err={cv_errors[-1]:.4f}  "
              f"Test_err={test_errors[-1]:.4f}  Cum_var={cum_var[nc-1]:.4f}")

    return cv_errors, test_errors, cum_var.tolist()


def pls_component_analysis(X_train, y_train, X_test, y_test, max_comp=None):
    if max_comp is None:
        max_comp = min(X_train.shape[1], 15)
...
def baseline_accuracy(X_train, y_train, X_test, y_test):
    ...
    return 1.0 - cv_acc, 1.0 - lr.score(X_test, y_test)


def plot_pca_pls(pca_cv, pca_test, pls_cv, pls_test, explained_vars,
                 bl_cv, bl_test, save_path="task2c_pca_pls.png"):
    ...
    # PCA error
    ax = axes[1]
    ax.plot(list(n_pca), pca_cv, "b-o", ms=4, label="PCA CV Error")
    ax.plot(list(n_pca), pca_test, "b--s", ms=4, label="PCA Test Error")
    ax.axhline(bl_cv, color="gray", ls=":", label=f"Baseline CV={bl_cv:.4f}")
    ax.axhline(bl_test, color="gray", ls="--", alpha=0.5,
               label=f"Baseline Test={bl_test:.4f}")
    opt_pca = np.argmin(pca_cv) + 1
    ax.axvline(opt_pca, color="red", ls=":", alpha=0.7,
               label=f"Optimal nc={opt_pca}")
    ax.set_xlabel("Number of Components")
    ax.set_ylabel("Misclassification Error")
    ax.set_title("PCA — Error vs Components")
    ...
    ax.plot(range(1, n_common+1), pca_cv[:n_common], "b-o", ms=4,
            label="PCA CV Error")
    ax.plot(range(1, n_common+1), pls_cv[:n_common], "r-s", ms=4,
            label="PLS CV Error")
    ax.plot(range(1, n_common+1), pca_test[:n_common], "b--o", ms=3,
            alpha=0.5, label="PCA Test")
    ax.plot(range(1, n_common+1), pls_test[:n_common], "r--s", ms=3,
            alpha=0.5, label="PLS Test")
    ax.axhline(bl_cv, color="gray", ls=":", label="Baseline CV")
    ax.set_xlabel("Number of Components")
    ax.set_ylabel("Misclassification Error")
    ...
    plt.suptitle("Task 2c — PCA & PLS (Credit Approval)", fontsize=14, y=1.02)
    plt.tight_layout()
...
def main():
    print("=" * 60)
    print("Task 2c: PCA and PLS (Credit Approval)")
    print("=" * 60)
    ...
    print("[0] Baseline (all features, no reduction) …")
    ...
    max_pls = min(n_feat, 15)
    print(f"\n[2] PLS Component Analysis (up to {max_pls}) …")
    pls_cv, pls_test = pls_component_analysis(
        X_train, y_train, X_test, y_test, max_comp=max_pls)

    print("\n[3] Plotting …")
    plot_pca_pls(pca_cv, pca_test, pls_cv, pls_test, expl_var,
                 bl_cv, bl_test)
    ...
    winner = "PLS" if pls_cv[opt_pls-1] < pca_cv[opt_pca-1] else "PCA"
    print(f"  → {winner} achieves lower CV error with fewer components")